...
def get_move_around_cam_T_cw(
    ...
    H,
    W,
    move_around_angle_deg, # actually already rad
    total_steps,
    center_id=None,
):
    ...
    for i in tqdm(range(total_steps)):
        if center_id is None:
            ...
            if i == 0:
                ...
                output = d_model(move_around_id)
                # avg_center = output[0].quantile(0.25, dim=0)
                # Leo Question: Why render can get the depth?
                # depth = (render_dict["dep"] / (render_dict["alpha"] + 1e-6))[0]
                depth = render_dict["dep"][0]
                ...
                avg_center_depth = torch.mean(patch)

                center_dep = depth[img_h // 2, img_w // 2].item()
                if center_dep < 1e-2:
                    center_dep = depth[render_dict["alpha"][0] > 0.1].min().item()
                print(f"center_dep: {center_dep}, avg_center: {avg_center_depth}")
                focus_point = torch.Tensor([0.0, 0.3, center_dep]).to(depth)
                # focus_point = avg_center.to(depth)
                focus_point[2] = 1.1
                # Leo Question: Why this can be the radius?
                move_around_radius = 1.5 \
                # (
                #     np.tan(move_around_angle_deg) * focus_point[2].item()
                # )

        theta = 4 * np.pi * i / (total_steps - 1)
        x = focus_point[0] + (
            move_around_radius * np.sin(theta) \
            # - move_around_radius
        )
        z = focus_point[2] + move_around_radius * np.cos(theta)
        y = focus_point[1] + 0.0
        # translation x, y, z
        T_c_new = torch.eye(4).to(cams.T_wc(0))
        T_c_new[0, -1] = x
        ...
        new_cam_center = T_c_new[:3, -1]
        _z_dir = F.normalize(focus_point[:3] - new_cam_center, dim=0)
        _x_dir = F.normalize(
            torch.cross(torch.Tensor([0.0, 1.0, 0.0]).to(_z_dir), _z_dir), dim=0
        )
        _y_dir = F.normalize(torch.cross(_z_dir, _x_dir), dim=0)
...
def viz_single_2d_camera_video(
    ...
    for cam_tid in tqdm(range(len(pose_list))):
        ...
        print("gs5 params:", gs5[1][0].shape, gs5[1][1].shape, gs5[1][2].shape, gs5[1][3].shape, gs5[1][4].shape)
