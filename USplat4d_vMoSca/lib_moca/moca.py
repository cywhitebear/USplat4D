...
def moca_solve(
    ...
    assert static_id_mode in ["raft", "track", "gt_masks"], f"{static_id_mode} not in [raft, track, gt_masks]"
    s2d.to(device)
    ...
    # * 1. mark static track
    if static_id_mode == "gt_masks" and s2d.has_gt_masks:
        logging.info(f"Use GT masks to mark static tracks")
        # Use GT masks to identify dynamic tracks
        with torch.no_grad():
            # Move track data to same device as gt_masks
            track_device = track.to(s2d.gt_masks.device)
            track_mask_device = track_mask.to(s2d.gt_masks.device)
        ...
        # GT masks are foreground (dynamic) regions
        # Tracks with high GT mask coverage are dynamic, low coverage are static
        dynamic_threshold = 0.5  # Threshold for considering a track as dynamic
        track_dynamic_selection = gt_mask_list.mean(0) >= dynamic_threshold
        track_static_selection = gt_mask_list.mean(0) < dynamic_threshold
        
        logging.warning(
            f"GT mask identify {track_static_selection.sum()} static, {track_dynamic_selection.sum()} dynamic out of {len(track_static_selection)} tracks"
        )
    elif static_id_mode == "raft":
