...
def compute_ssim_map(
    ...
    # Use CPU device to avoid GPU cleanup issues and segfaults # Previously I don't have bugs here, but now I have to use CPU
    tmp_device = jax.devices("cpu")[0]
    with jax.default_device(tmp_device):
        # Convert numpy arrays to JAX arrays INSIDE device context   # my code after have bugs
        ...
        def convolve2d(z, m, f):
            z_ = jsp.signal.convolve2d(
                z * m, f, mode="same", precision=jax.lax.Precision.HIGHEST
            )
        ...
        # Convert JAX arrays back to NumPy to release GPU device context # my code after have bugs
        ssim_map_np = np.array(ssim_map)
        ssim_np = np.array(ssim)
        
        # Explicitly delete JAX arrays to free GPU memory # my code after have bugs
        del ssim_map, ssim, img0, img1, mask
        del mu0, mu1, mu00, mu11, mu01
        del sigma00, sigma11, sigma01
        del filt, numer, denom

    # return ssim_map, ssim
    return ssim_map_np, ssim_np # my code after have bugs

@torch.inference_mode()
def compute_lpips_map(
    ...
    lpips_map =lpips_model(img0, img1).cpu().numpy()[0, 0, ..., None]
    return lpips_map, masked_mean(lpips_map, mask)