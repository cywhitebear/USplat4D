...
def c2w_to_w2c(c2w: torch.Tensor) -> torch.Tensor:
    """
    Convert a camera-to-world transformation matrix to world-to-camera.

    Args:
        c2w (torch.Tensor): (4, 4) camera-to-world transformation matrix.

    Returns:
        torch.Tensor: (4, 4) world-to-camera transformation matrix.
    """
    assert c2w.shape == (4, 4), "Input must be a 4x4 matrix"

    R = c2w[:3, :3]        # Rotation
    t = c2w[:3, 3:]        # Translation column (3x1)
    ...
    w2c[:3, :3] = R_inv
    w2c[:3, 3] = t_inv.squeeze()

    return w2c

def load_all_tto_poses(folder): # my code
    import os
    import numpy as np
    import glob
    # Find all matching files and sort by number
    pose_files = sorted(
        glob.glob(os.path.join(folder, "tto_test_pose_*.npz")),
    ...
    return all_poses_list  # shape: [T] [N1, 4, 4] [N2, 4, 4] ...
...
def photometric_warmup(ws, log_path, fit_cfg):
    ...
    # GT camera pose training control
    disable_cam_opt = getattr(fit_cfg, "disable_cam_pose_training", False)
    warm_lr_cam_q = getattr(fit_cfg, "photo_warm_lr_cam_q", 0.00003)
    warm_lr_cam_t = getattr(fit_cfg, "photo_warm_lr_cam_t", 0.00003)
    warm_optim_cam_after_steps = getattr(fit_cfg, "photo_warm_optim_cam_after_steps", 0)
    
    if disable_cam_opt:
        logging.info("Camera pose training disabled: freezing warmup camera parameters")
        warm_lr_cam_q = 0.0
        warm_lr_cam_t = 0.0
        warm_optim_cam_after_steps = PHOTO_STATIC_WARM_STEPS + 1
    ...
    photo_solver.photometric_fit(
        ...
        optim_cam_after_steps=warm_optim_cam_after_steps,
        ...
        optimizer_cfg=OptimCFG(
            lr_cam_f=0.0,
            lr_cam_q=warm_lr_cam_q,
            lr_cam_t=warm_lr_cam_t,
            # gs
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    # Add GT mask support for dynamic curve extraction when using GT depth
    gt_mask_for_curves = None
    refilter_2d_track_flag = True
    if use_gt_masks and hasattr(s2d, "gt_masks"):
        refilter_2d_track_flag = False
        # Sample GT masks at track locations to create track-level masks
        from lib_mosca.dynamic_solver_utils import round_int_coordinates, query_image_buffer_by_pix_int_coord
        
        # Get tracks for dynamic regions
        track = s2d.track[:, s2d.dynamic_track_mask]  # [T, N, 2]
        
        # Sample GT masks at track locations
        gt_track_masks = []
        device = s2d.track.device  # Get device from existing tensors
        for t in range(s2d.T):
            _uv = track[t]  # [N, 2]
            _int_uv, _inside_mask = round_int_coordinates(_uv, s2d.H, s2d.W)
            _gt_mask_values = query_image_buffer_by_pix_int_coord(s2d.gt_masks[t].clone().to(device), _int_uv)  # [N]
            gt_track_masks.append(_gt_mask_values)
        
        gt_mask_for_curves = torch.stack(gt_track_masks, 0)  # [T, N]
        # Convert to boolean type to match track_mask
        gt_mask_for_curves = gt_mask_for_curves.bool()
        logging.info(f"GT mask sampling: {gt_mask_for_curves.sum()} / {gt_mask_for_curves.numel()} track points are in dynamic regions")
    
    curve_xyz, curve_mask, curve_rgb, curve_filter_mask = get_dynamic_curves(
        s2d,
        cams,
        t_list=sub_t_list,
        refilter_2d_track_flag=refilter_2d_track_flag,
        ...
        * get_dynamic_curves_filter_factor,
        ...
        fg_additional_mask=gt_mask_for_curves,  # Pass GT masks for dynamic regions
    )
...
def photometric_reconstruct(ws, log_path, fit_cfg, dir_saving_this_run_model):#, graph_dir):
    seed_everything(SEED)
    DEPTH_DIR, TAP_MODE = auto_get_depth_dir_tap_mode(ws, fit_cfg)
    DEPTH_BOUNDARY_TH = getattr(fit_cfg, "depth_boundary_th", 1.0)
    DEP_MEDIAN = getattr(fit_cfg, "dep_median", 1.0)
    ...
    photo_solver = DynReconstructionSolver(
        working_dir=log_path,
        dir_saving_this_run_model=dir_saving_this_run_model,
        device=device,
        radius_init_factor=getattr(fit_cfg, "gs_radius_init_factor", 4.0),
        opacity_init_factor=getattr(fit_cfg, "gs_opacity_init_factor", 0.95),
    )
    ...
    # ugraph_model = photo_solver.get_graph_model(
    #     state_dict={}, # TODO!!
    #     graph_dir=graph_dir,
    #     device=device,
    # )
    ...
    # GT camera pose training control for photometric fit
    disable_cam_opt = getattr(fit_cfg, "disable_cam_pose_training", False)
    photo_total_steps = getattr(fit_cfg, "photo_total_steps", 6000)
    photo_optim_cam_after_steps = getattr(fit_cfg, "photo_optim_cam_after_steps", 0)
    photo_lr_cam_q = getattr(fit_cfg, "photo_lr_cam_q", 0.00003)
    photo_lr_cam_t = getattr(fit_cfg, "photo_lr_cam_t", 0.00003)
    
    if disable_cam_opt:
        logging.info("Camera pose training disabled: freezing photometric camera parameters")
        photo_lr_cam_q = 0.0
        photo_lr_cam_t = 0.0
        photo_optim_cam_after_steps = photo_total_steps + 1

    photo_solver.photometric_fit(
        s2d=s2d,
        total_steps=photo_total_steps,
        optim_cam_after_steps=photo_optim_cam_after_steps,
        ...
        optimizer_cfg=OptimCFG(
            lr_cam_f=0.0,
            lr_cam_q=photo_lr_cam_q,
            lr_cam_t=photo_lr_cam_t,
            # gs
...
def photometric_reconstruct_from_pretrained(ws, log_path, fit_cfg, graph_dir, loaded, dir_saving_this_run_model, device="cuda:0"):
    seed_everything(SEED)
    DEPTH_DIR, TAP_MODE = auto_get_depth_dir_tap_mode(ws, fit_cfg)
    DEPTH_BOUNDARY_TH = getattr(fit_cfg, "depth_boundary_th", 1.0)
    DEP_MEDIAN = getattr(fit_cfg, "dep_median", 1.0)

    d_model:DynSCFGaussian=loaded["d_model"]
    s_model:StaticGaussian=loaded["s_model"]
    ...
    print("Loading solved camera and s2d done!")
    print('='*20)

    # construct the GS models
    print('='*20)
    print("Constructing DynReconstructionSolver...")
    photo_solver = DynReconstructionSolver(
        working_dir=log_path,
        dir_saving_this_run_model=dir_saving_this_run_model,
        device=device,
    ...
    if ugraph_model is not None:
        print("Loaded ugraph_model from saved d_model.")
        print("For ugraph+gen version")
    elif graph_dir is not None:
        print("Getting ugraph_model")
        print("For ugraph version")
        state_dict={}
        ugraph_model = photo_solver.get_graph_model(
            state_dict=state_dict, # TODO!!
            graph_dir=graph_dir,
            device=device,
        )

        # manual cull BEGIN
        if 'precompute.params.transls' in state_dict.keys():
            print("Dont cull in initialzation due to using graph model in ckpt")
        elif 'dists_key' not in ugraph_model.precompute.params.keys():
            print("Dont cull in initialzation due to using ini.ckpt")
        else:
            print("Cull in initialzation!!!")
            ids_key_nonkey=ugraph_model.precompute.graph_para['ids_key_nonkey']
            n_fg_old=d_model.N
            should_cull_fg=torch.ones(n_fg_old, dtype=torch.bool, device=device)
            should_cull_fg[ids_key_nonkey]=False

            # optimizer has not been optimized yet.
            d_model._prune_points2(mask=should_cull_fg)
            logging.info(f"Prune: {should_cull_fg.sum()}")
            logging.info(f"Prune: {n_fg_old}->{d_model.N}")

        # manual cull END
    else:
        ugraph_model=None

    print("Done DynReconstructionSolver Construction!")
    print('='*20)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--graph_dir", type=str, help="graph_dir folder", required=False)
    parser.add_argument("--log_dir", type=str, help="log_dir folder", required=False)
    parser.add_argument("--no_viz", action="store_true", help="no viz")
    parser.add_argument("--use_gt_masks", action="store_true", help="use gt masks")
    parser.add_argument("--gt_mask_path", type=str, help="gt mask path", default="mask")
    parser.add_argument("--dir_name_saving_this_run_model", type=str, help="saved_ugraph_model or saved_ugraph_model_gen", required=False)
    parser.add_argument("--dir_name_saved_model", type=str, help="dir_name_saved_model", required=False)
    parser.add_argument("--run_ugraph", action="store_true", help="run ugraph")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use GT camera poses without further optimization",
    )
    ...
    if args.graph_dir is not None:
        dir_saving_this_run_model=osp.join(args.graph_dir,args.dir_name_saving_this_run_model)
    elif args.log_dir is not None:
        dir_saving_this_run_model=osp.join(args.log_dir,args.dir_name_saving_this_run_model)

    if args.log_dir is None:
        # mosca train from scratch
        cfg = OmegaConf.load(args.cfg)
        cli_cfg = OmegaConf.from_dotlist([arg.lstrip("--") for arg in unknown])
        cfg = OmegaConf.merge(cfg, cli_cfg)
        
        # Override config with command line arguments for mask settings
        if args.use_gt_masks:
            cfg.use_gt_masks = True
        if args.gt_mask_path != "mask":  # Only override if explicitly set
            cfg.gt_mask_path = args.gt_mask_path
        
        if args.disable_cam_pose_training:
            cfg.disable_cam_pose_training = True

        logdir = setup_recon_ws(args.ws, fit_cfg=cfg, logdir="logs2") ########## "logs"
        dir_saving_this_run_model=osp.join(logdir,args.dir_name_saving_this_run_model)
        ...
        start_time = time.time()
        photometric_warmup(args.ws, logdir, cfg)  # this is optional, if not set, will directly skip and return.
        execution_times.append(time.time() - start_time)
        ...
        start_time = time.time()
        photometric_reconstruct(args.ws, logdir, cfg, dir_saving_this_run_model) #, args.graph_dir)
        execution_times.append(time.time() - start_time)

        # Save execution times to a text file in logdir
        function_names = ['static_reconstruct', 'photometric_warmup', 'scaffold_reconstruct', 'photometric_reconstruct']
        runtime_file = os.path.join(logdir, 'execution_times.txt')
        with open(runtime_file, 'w') as f:
            f.write("Function Execution Times\n")
            f.write("========================\n\n")
            for name, exec_time in zip(function_names, execution_times):
                f.write(f"{name}: {exec_time:.4f} seconds\n")
            
            # Write total execution time
            total_time = sum(execution_times)
            f.write(f"\nTotal execution time: {total_time:.4f} seconds\n")

        print(f"Execution times have been saved to: {runtime_file}")

    elif args.run_ugraph is False:
        print("run original mosca from step 4.")
        print("*"*30)
        # mosca using pretrained model.
        device="cuda:0"
        logdir = args.log_dir
        print("logdir is given:", logdir)
        cfg, d_model, s_model, cams = load_model_cfg(args.cfg, saved_dir=args.log_dir,device="cuda:0",is_eval=False,use_ugraph=False)
        loaded = {
            "d_model": d_model,
            "s_model": s_model,
            "cams": cams,
        }
        setattr(cfg, "photo_total_steps", 9601) # 8000 # 1600
        # setattr(cfg, "photo_d_ctrl_start_ratio", 0.01)
        # setattr(cfg, "photo_d_ctrl_end_ratio", 0.8)
        # setattr(cfg, "photo_d_ctrl_prune_opacity_th", 0.01) # 0.01
    ...
    elif args.run_ugraph:
        print("run ugraph.")
        print("*"*30)
        # ugraph using pretrained model
        device="cuda:0"
        logdir = args.log_dir
        print("logdir is given:", logdir)
        if args.dir_name_saved_model is not None:
            pth_saved_dir= osp.join(args.graph_dir,args.dir_name_saved_model)
            use_ugraph=True
        elif args.relative_dir_saved_mosca_model is not None:
            pth_saved_dir= osp.join(logdir,args.relative_dir_saved_mosca_model)
            use_ugraph=False # cull to build new ugraph
        else:
            pth_saved_dir= logdir
            use_ugraph=False # cull to build new ugraph
        print("pth will be loaded from:", pth_saved_dir)
        cfg, d_model, s_model, cams = load_model_cfg(args.cfg, saved_dir=pth_saved_dir,device=device,is_eval=False,use_ugraph=use_ugraph)
        loaded = {
            "d_model": d_model,
            "s_model": s_model,
            "cams": cams,
        }
        # cfg.photo_total_steps = 7000
        setattr(cfg, "photo_total_steps", 1600) # 8000 # 3200 1600
        setattr(cfg, "photo_d_ctrl_start_ratio", 0.01)
        setattr(cfg, "photo_d_ctrl_end_ratio", 0.8)
        setattr(cfg, "photo_d_ctrl_prune_opacity_th", 0.01) # 0.01
        setattr(cfg, "photo_d_ctrl_densify_steps", 300) # 300
        ...
        # Save execution times to a text file in logdir
        function_names = ['photometric_reconstruct_from_pretrained']
        runtime_file = os.path.join(args.graph_dir, 'execution_times.txt')
        with open(runtime_file, 'w') as f:
            f.write("Function Execution Times\n")
            f.write("========================\n\n")
            for name, exec_time in zip(function_names, execution_times):
    ...
    logdir=osp.join(dir_saving_this_run_model, "step_1599") 
    ...
    if datamode in ["iphone"]:
        try:
            seq_name = osp.basename(osp.normpath(args.ws))
            pck005=test_pck(
                saved_dir=logdir,
