...
from lib_ugraph.ugraph_model4 import get_graph_rigidity_interpolation_loss, get_DQB_nonkeyGaussian # graph_model3/4
...
def test_main(
    ...
    try:
        ...
        print("UGraph model loaded")
    except:
        print("UGraph model not loaded")
        ugraph_model = None
...
def main_ugraph():
    # modified from in the end of mosca_reconstruct.py
    import argparse
    # from mosca_cfg import load_model_cfg

    parser = argparse.ArgumentParser("Ugraph-MoSca-V2 Reconstruction")
    ...
    parser.add_argument("--pth_save_dir", type=str, help="pth_save_dir folder", required=False)
    args, unknown = parser.parse_known_args()

    # logdir = args.log_dir
    cfg = OmegaConf.load(args.cfg)
    pth_save_dir = args.pth_save_dir

    # if args.graph_dir is None:
    #     # mosca using pretrained model.
    #     device="cuda:0"
    #     print("logdir is given:", logdir)
    #     cfg, d_model, s_model, cams = load_model_cfg(args.cfg, saved_dir=args.log_dir,device=device,is_eval=False,use_ugraph=False)
    #     # loaded = {
    #     #     "d_model": d_model,
    #     #     "s_model": s_model,
    #     #     "cams": cams,
    #     # }
    # else:
    #     assert args.pth_save_dir is not None and logdir!=''
    #     # cfg, d_model, s_model, cams = load_model_cfg(args.cfg, args.pth_save_dir, use_ugraph=True)
    #     # ugraph_model = d_model.ugraph_model
    ...
    if datamode in ["iphone"]:
        ...
        if True:
            ...
            test_pck(
                saved_dir=pth_save_dir,
                gt_npz_fn=f"./eval_utils/pck_gt_packs/{seq_name}_train_pck.npz",
                device=torch.device("cuda:0"),
                save_fn=osp.join(pth_save_dir, "pck5.txt"),
            )
        # except:
        #     logging.warning("PCK5 failed")
        #     pass

    test_fps(saved_dir=pth_save_dir, rounds=1 if datamode in ["iphone"] else 3)
    ...
    if not args.no_viz and datamode in ["wild"]:
        ...
        viz_main(
            save_dir=osp.join(pth_save_dir, "viz"),
            log_dir=pth_save_dir,
            cfg_fn=args.cfg,
...
def main3():
    # quick test
    # saved_dir="/data/dataset/iphone_mosca_trained/apple/logs/iphone_fit_native_add3/saved_ugraph_model/step_1599"
    saved_dir="/data/dataset/iphone_mosca_trained/apple/logs/iphone_fit_native_add3"
    data_root="/data/dataset/iphone_mosca_trained/apple"
    gt_dir = osp.join(data_root, "test_images")
    eval_prefix='tto_'
    eval_also_dyncheck_non_masked=False
    eval_dycheck(
