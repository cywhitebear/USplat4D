...
class CameraPath:
    def __init__(
        self, server: viser.ViserServer, duration_element: viser.GuiInputHandle[float] | None
    ):
        self._server = server
        ...
        self.show_spline: bool = False # True
...
def populate_render_tab(
    ...
    def _(event: viser.GuiEvent) -> None:
        assert event.client is not None
        camera_path_dir = datapath.parent / "camera_paths"
        camera_path_dir.mkdir(parents=True, exist_ok=True)
        print(f"camera_path_dir: {camera_path_dir}")
