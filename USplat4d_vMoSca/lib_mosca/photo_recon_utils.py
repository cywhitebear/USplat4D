...
def apply_gs_control_d(
    ...
    last_N=None,
    record_flag=True,
    ugraph_model:UGraphGaussian | None = None,
    ...
    if (
        ...
        logging.info(f"GS Control at {step}")
    if step in gs_control_cfg.densify_steps:
        if ugraph_model is None: # for original mosca dynamic model
            N_old = model.N
            densify_mask=model.densify(
                optimizer=optimizer_gs,
                max_grad=gs_control_cfg.densify_max_grad, # TODO
                percent_dense=gs_control_cfg.densify_percent_dense,
                extent=0.5,
                verbose=True,
            )
        ...
        else: # for ugraph-based dynmaic model
            N_old = model.N
            # keep key gaussians
            keep_mask = torch.zeros(model.N, dtype=torch.bool, device=device)
            keep_mask[ugraph_model.precompute.graph_para['ids_key']]=True # my test
            # keep_mask = None

            selected_clone_pts_mask,selected_split_pts_mask=model.densify(
                optimizer=optimizer_gs,
                max_grad=gs_control_cfg.densify_max_grad, #0.0, #
                percent_dense=gs_control_cfg.densify_percent_dense,
                extent=0.5,
                verbose=True,
                no_densify_mask=keep_mask
            )
            logging.info(f"Densify: {N_old}->{model.N}")
            # clone ugraph
            N_ugraph_old = ugraph_model.precompute.params['transls'].shape[1]
            ugraph_model._densify_points(
                optimizers_ugraph,
                should_fg_densify=selected_clone_pts_mask,
            )
            logging.info(f"Clone ugraph_model: {N_ugraph_old}->{ugraph_model.precompute.params['transls'].shape[1]}")
            # split ugraph
            N_ugraph_old2 = ugraph_model.precompute.params['transls'].shape[1]
            ugraph_model._densify_points(
                optimizers_ugraph,
                should_fg_densify=selected_split_pts_mask,
            )
            logging.info(f"Split ugraph_model: {N_ugraph_old2}->{ugraph_model.precompute.params['transls'].shape[1]}")
            assert model.N == ugraph_model.precompute.params['transls'].shape[1]
    if step in gs_control_cfg.prune_steps:
        if ugraph_model is None: # static model
            N_old = model.N
            prune_mask=model.prune_points(
                optimizer_gs,
                min_opacity=gs_control_cfg.prune_opacity_th,
            ...
            # cull ugraph
            N_ugraph_old = ugraph_model.precompute.params['transls'].shape[1]
            ugraph_model._prune_points(
                optimizers_ugraph,
                should_fg_cull=prune_mask,
            )
            logging.info(f"Prune ugraph_model: {N_ugraph_old}->{ugraph_model.precompute.params['transls'].shape[1]}")
    if step in gs_control_cfg.reset_steps:
        model.reset_opacity(optimizer_gs, gs_control_cfg.reset_opacity)
        # TODO: reset ugraph optimizers as well
    return
...
class OptimCFG:
    def __init__(
        ...
        # # graph
        self.lr_transls = lr_transls
        self.lr_oris_wxyz = lr_oris_wxyz
        self.lr_dists_key = lr_dists_key
        self.lr_dists_oris_key = lr_dists_oris_key
        self.lr_weight_dist_key = lr_weight_dist_key
        self.lr_dists_nonkey = lr_dists_nonkey
        self.lr_dists_oris_nonkey = lr_dists_oris_nonkey
        self.lr_weight_dist_nonkey = lr_weight_dist_nonkey
        self.lr_dir_matrix_key = lr_dir_matrix_key
        self.lr_weight_dir_key = lr_weight_dir_key
        self.lr_dir_matrix_nonkey = lr_dir_matrix_nonkey
        self.lr_weight_dir_nonkey = lr_weight_dir_nonkey
        self.lr_weight_param_key = lr_weight_param_key
        self.lr_src_xyz_1_nonkey = lr_src_xyz_1_nonkey
        self.lr_src_xyzw_1_nonkey = lr_src_xyzw_1_nonkey
        self.lr_per_nonkeyGaussian_dweight_nonkey = lr_per_nonkeyGaussian_dweight_nonkey
        self.lr_per_nonkeyGaussian_dxyz_dxyzw = lr_per_nonkeyGaussian_dxyz_dxyzw
    ...
    @property
    def get_ugraph_lr_dict(self): # my
        return {
            "lr_transls": self.lr_transls,
            "lr_oris_wxyz": self.lr_oris_wxyz,
            "lr_dists_key": self.lr_dists_key,
            "lr_dists_oris_key": self.lr_dists_oris_key,
            "lr_weight_dist_key": self.lr_weight_dist_key,
            "lr_dists_nonkey": self.lr_dists_nonkey,
            "lr_dists_oris_nonkey": self.lr_dists_oris_nonkey,
            "lr_weight_dist_nonkey": self.lr_weight_dist_nonkey,
            "lr_dir_matrix_key": self.lr_dir_matrix_key,
            "lr_weight_dir_key": self.lr_weight_dir_key,
            "lr_dir_matrix_nonkey": self.lr_dir_matrix_nonkey,
            "lr_weight_dir_nonkey": self.lr_weight_dir_nonkey,
            "lr_weight_param_key": self.lr_weight_param_key,
            "lr_src_xyz_1_nonkey": self.lr_src_xyz_1_nonkey,
            "lr_src_xyzw_1_nonkey": self.lr_src_xyzw_1_nonkey,
            "lr_per_nonkeyGaussian_dweight_nonkey": self.lr_per_nonkeyGaussian_dweight_nonkey,
            "lr_per_nonkeyGaussian_dxyz_dxyzw": self.lr_per_nonkeyGaussian_dxyz_dxyzw,
        }
    ...
    def get_scheduler(self, total_steps):
        ...
        # graph_scheduling_dict = {
        #     "lr_transls": get_expon_lr_func(
        #         lr_init=self.lr_transls,
        #         lr_final=self.lr_transls / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_oris_wxyz": get_expon_lr_func(
        #         lr_init=self.lr_oris_wxyz,
        #         lr_final=self.lr_oris_wxyz / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_dists_key": get_expon_lr_func(
        #         lr_init=self.lr_dists_key,
        #         lr_final=self.lr_dists_key / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_dists_oris_key": get_expon_lr_func(
        #         lr_init=self.lr_dists_oris_key,
        #         lr_final=self.lr_dists_oris_key / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_weight_dist_key": get_expon_lr_func(
        #         lr_init=self.lr_weight_dist_key,
        #         lr_final=self.lr_weight_dist_key / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_dists_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_dists_nonkey,
        #         lr_final=self.lr_dists_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_dists_oris_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_dists_oris_nonkey,
        #         lr_final=self.lr_dists_oris_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_weight_dist_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_weight_dist_nonkey,
        #         lr_final=self.lr_weight_dist_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_dir_matrix_key": get_expon_lr_func(
        #         lr_init=self.lr_dir_matrix_key,
        #         lr_final=self.lr_dir_matrix_key / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_weight_dir_key": get_expon_lr_func(
        #         lr_init=self.lr_weight_dir_key,
        #         lr_final=self.lr_weight_dir_key / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_dir_matrix_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_dir_matrix_nonkey,
        #         lr_final=self.lr_dir_matrix_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_weight_dir_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_weight_dir_nonkey,
        #         lr_final=self.lr_weight_dir_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_weight_param_key": get_expon_lr_func(
        #         lr_init=self.lr_weight_param_key,
        #         lr_final=self.lr_weight_param_key / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_src_xyz_1_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_src_xyz_1_nonkey,
        #         lr_final=self.lr_src_xyz_1_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_src_xyzw_1_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_src_xyzw_1_nonkey,
        #         lr_final=self.lr_src_xyzw_1_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        #     "lr_per_nonkeyGaussian_dweight_nonkey": get_expon_lr_func(
        #         lr_init=self.lr_per_nonkeyGaussian_dweight_nonkey,
        #         lr_final=self.lr_per_nonkeyGaussian_dweight_nonkey / 10,
        #         lr_delay_mult=0.01,
        #         max_steps=total_steps,
        #     ),
        # }
