...
class DynSCFGaussian(nn.Module):
    ...
    def __init__(
        ...
        self.ugraph_model = None

        return

    def set_ugraph(self, ugraph_model):
    ...
    def load_from_ckpt(cls, ckpt, device=torch.device("cuda:0")):
        ...
        filtered_ckpt = {k: v for k, v in ckpt.items() if not k.startswith("ugraph_model.")}
    ...
    def forward(self, t: int, active_sph_order=None, nn_fusion=None, use_ugraph=True):
        ...
        if not use_ugraph:
            ...
            transls_trainable_t=self.ugraph_model.precompute.params['transls'][t]
            oris_wxyz_trainable_t=self.ugraph_model.precompute.params['oris_wxyz'][t]
            mu_live=transls_trainable_t
            fr_live=q2R(oris_wxyz_trainable_t) # wxyz # CHECK
            # mu_live = self.ugraph_model.precompute.graph_para['transls_interpolation'][t]
            # fr_live = q2R(self.ugraph_model.precompute.graph_para['oris_interpolation'][t]) # wxyz
    ...
    def _densify_and_clone(self, optimizer, grad_norm, grad_threshold, scale_th, no_densify_mask=None):
        ...
        if no_densify_mask is not None:
            selected_pts_mask = torch.logical_and(selected_pts_mask, ~no_densify_mask)
        ...
        return new_xyz.shape[0],selected_pts_mask
    ...
    def _densify_and_split2(
        self,
        optimizer,
        grad_norm,
        grad_threshold,
        scale_th,
        N=2,  # now we add 1 new point and overwrite originals, so N=2
        no_densify_mask=None
    ):
        # my code: 
        # Instead of removing the old splited points and add 2 (N=2) copy in the end,
        # we want to keep their position but assign its new value and add 1 copy in the end.
        #####################################################
        # TODO: check whether this has a bug, seems very noisy
        # Extract points that satisfy the gradient condition
        _scaling = self.get_s
        ...
        # Create N new points (N=2); originals will be overwritten in-place
        stds = _scaling[selected_pts_mask].repeat(N, 1)
        means = torch.zeros((stds.size(0), 3), device=self.device)
        samples = torch.normal(mean=means, std=stds)
        rots = quaternion_to_matrix(self._rotation[selected_pts_mask]).repeat(N, 1, 1)
        # disturbed_xyz = torch.einsum("nij,nj->ni", rots, samples) + self._xyz[selected_pts_mask].repeat(N, 1)
        disturbed_xyz = self._xyz[selected_pts_mask].repeat(N, 1)

        new_scaling = _scaling[selected_pts_mask].repeat(N, 1) / (0.8 * (N + 1))
        new_scaling = torch.clamp(new_scaling, max=self.max_scale, min=self.min_scale)
        new_scaling = self.s_inv_act(new_scaling)

        # Overwrite selected Gaussians in-place with disturbed version
        with torch.no_grad():
            self._xyz.data[selected_pts_mask] = disturbed_xyz[:num_selected]
            self._scaling.data[selected_pts_mask] = new_scaling[:num_selected]
        # self._xyz[selected_pts_mask] = disturbed_xyz[:num_selected]
        # self._scaling[selected_pts_mask] = new_scaling[:num_selected]
        # self._rotation[selected_pts_mask] = self._rotation[selected_pts_mask]
        # self._features_dc[selected_pts_mask] = self._features_dc[selected_pts_mask]
        # self._features_rest[selected_pts_mask] = self._features_rest[selected_pts_mask]
        # self._opacity[selected_pts_mask] = self._opacity[selected_pts_mask]
        # self._dynamic_logit[selected_pts_mask] = self._dynamic_logit[selected_pts_mask]
        # self._skinning_weight[selected_pts_mask] = self._skinning_weight[selected_pts_mask]

        # Now add 1 copy (i.e., N=1)
        added_xyz = disturbed_xyz[num_selected:]  # same as disturbed one
        added_scaling = new_scaling[num_selected:]
        added_rotation = self._rotation[selected_pts_mask].clone()
        added_features_dc = self._features_dc[selected_pts_mask].clone()
        added_features_rest = self._features_rest[selected_pts_mask].clone()
        added_opacities = self._opacity[selected_pts_mask].clone()
        added_dyn_logit = self._dynamic_logit[selected_pts_mask].clone()
        added_skinning_w = self._skinning_weight[selected_pts_mask].clone()

        self._densification_postprocess(
            optimizer,
            new_xyz=added_xyz,
            new_r=added_rotation,
            new_s_logit=added_scaling,
            new_o_logit=added_opacities,
            new_sph_dc=added_features_dc,
            new_sph_rest=added_features_rest,
            new_skinning_w=added_skinning_w,
            new_dyn_logit=added_dyn_logit,
        )

        # Update leaf buffer for added points
        new_attach_ind = self.attach_ind[selected_pts_mask].repeat(N-1) #
        new_ref_time = self.ref_time[selected_pts_mask].repeat(N-1)
        self.attach_ind = torch.cat([self.attach_ind, new_attach_ind], dim=0)
        self.ref_time = torch.cat([self.ref_time, new_ref_time], dim=0)

        return added_xyz.shape[0], selected_pts_mask
    ...
    def densify(
        ...
        n_clone,selected_clone_pts_mask = self._densify_and_clone(
            optimizer, grads, max_grad, percent_dense * extent, no_densify_mask=no_densify_mask
        )
        # don't clone the split ones
        if no_densify_mask is not None:
            no_densify_mask2 = torch.logical_or(no_densify_mask,selected_clone_pts_mask)
        else:
            no_densify_mask2 = selected_clone_pts_mask
        # self.N already increase. increase the length of mask as well.
        # avoid split the just-now cloned points
        pad_len = self.N - no_densify_mask2.shape[0]
        if pad_len > 0:
            pad = torch.ones(pad_len, dtype=no_densify_mask2.dtype, device=no_densify_mask2.device)
            no_densify_mask2 = torch.cat([no_densify_mask2, pad], dim=0)
        ## no_densify_mask2=None # orignal mosca code logic
        ## n_split,selected_split_pts_mask = self._densify_and_split(
        ##     optimizer, grads, max_grad, percent_dense * extent, N=2, no_densify_mask=no_densify_mask2
        ## )
        n_split,selected_split_pts_mask = self._densify_and_split2(
            optimizer, grads, max_grad, percent_dense * extent, N=2, no_densify_mask=no_densify_mask2
        )
        # selected_split_pts_mask=selected_split_pts_mask[:old_N] # for densify ugraph. To keep dim, we have [:old_N]. Because we only clone and split the old points (here, key points are excluded for ugraph)
        # assert torch.logical_and(selected_clone_pts_mask,selected_split_pts_mask).any() == False
        assert torch.logical_and(selected_clone_pts_mask,selected_split_pts_mask[:old_N]).any() == False
        # selected_densify_pts_mask = torch.logical_or(selected_clone_pts_mask,selected_split_pts_mask)
        # selected_densify_pts_mask = selected_clone_pts_mask #test

        # TODO
        # selected_clone_pts_ids = torch.where(selected_clone_pts_mask)[0]
        # selected_split_pts_ids = torch.where(selected_split_pts_mask)[0]
        # selected_densify_pts_ids = torch.cat([selected_clone_pts_ids,selected_split_pts_ids])

        if verbose:
            logging.info(f"Densify: Clone[+] {n_clone}, Split[+] {n_split}")
            # logging.info(f"Densify: Clone[+] {n_clone}")
        # torch.cuda.empty_cache()
        # return selected_densify_pts_mask
        # return selected_densify_pts_ids
        return selected_clone_pts_mask,selected_split_pts_mask
    ...
    def prune_points(
        ...
        if keep_mask is not None: # my code
            prune_mask = torch.logical_and(~keep_mask, prune_mask)
    ...
    def replace_param(self, name: str, new_tensor: torch.Tensor):
        """Safely replace a registered nn.Parameter with a new tensor."""
        old_param = getattr(self, name)
        new_param = torch.nn.Parameter(new_tensor.detach().to(old_param.device))
        delattr(self, name)
        self.register_parameter(name, new_param)

    def _prune_points2(self, mask): # my code
        valid_points_mask = ~mask
        if valid_points_mask.all():
            return

        # didn't work: TypeError: cannot assign 'torch.cuda.FloatTensor' as parameter '_xyz' (torch.nn.Parameter or None expected)
        ...
        # self._skinning_weight = self._skinning_weight[valid_points_mask]
        # self._dynamic_logit = self._dynamic_logit[valid_points_mask]


        
        self.replace_param("_xyz", self._xyz[valid_points_mask])
        self.replace_param("_features_dc", self._features_dc[valid_points_mask])
        self.replace_param("_features_rest", self._features_rest[valid_points_mask])
        self.replace_param("_opacity", self._opacity[valid_points_mask])
        self.replace_param("_scaling", self._scaling[valid_points_mask])
        self.replace_param("_rotation", self._rotation[valid_points_mask])
        self.replace_param("_skinning_weight", self._skinning_weight[valid_points_mask])
        self.replace_param("_dynamic_logit", self._dynamic_logit[valid_points_mask])
