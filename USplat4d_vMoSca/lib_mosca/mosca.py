...
def __DQB_warp2__(
    ...
    sk_src_node_xyz,
    sk_src_node_quat,
    sk_dst_node_xyz, # [N,K,3]
    sk_dst_node_quat, # [N,K,4]
    dyn_o=None,
    src_quat=None, # [N,4] # wxyz
):
    src_R=q2R(src_quat) # [N,3,3]
    sk_R_tq, sk_t_tq = __compute_delta_Rt_ji__(
        R_wj=q2R(sk_dst_node_quat),
        t_wj=sk_dst_node_xyz,
        R_wi=q2R(sk_src_node_quat),
    ...
    if src_R is not None:
        fr_dst = torch.einsum("nij,njk->nik", R_tq, src_R)
        quat_dst = matrix_to_quaternion(fr_dst)
    else:
        fr_dst = None
        quat_dst = None
    return mu_dst, quat_dst # [N,3], [N,4]

def __LBS_warp2__(
    sk_w,
    src_xyz,
    sk_src_node_xyz,
    sk_src_node_quat, # wxyz
    sk_dst_node_xyz,
    sk_dst_node_quat, # wxyz
    dyn_o=None,
    src_quat=None,
):
    src_R=q2R(src_quat) # [N,3,3]
    ...
    sk_quat_tq = matrix_to_quaternion(sk_R_tq) # [wxyz]
