...
def setup_recon_ws(ws, fit_cfg, logdir="logs"):
    ...
    backup_dir = osp.join(log_path, "src_backup_ugraph")
...
def update_s2d_track_identification(
    ...
    # Use GT masks if available, otherwise use EPI
    if use_gt_masks and s2d.has_gt_masks:
        # Use GT masks to identify dynamic tracks
        with torch.no_grad():
            # Move track data to same device as gt_masks
            track_device = s2d.track.to(s2d.gt_masks.device)
            track_mask_device = s2d.track_mask.to(s2d.gt_masks.device)
            
            gt_mask_list = query_buffers_by_track(
                s2d.gt_masks[..., None], track_device, track_mask_device
            )
            gt_mask_list = gt_mask_list.squeeze(-1).cpu()
        
        # GT masks are foreground (dynamic) regions, so invert for static selection
        epi_track_static_selection = gt_mask_list.mean(0) < 0.5  # Mostly static
        epi_track_dynamic_selection = gt_mask_list.mean(0) >= 0.5  # Mostly dynamic
    else:
...
def set_epi_mask_to_s2d_for_bg_render(s2d, epi_th, device, use_gt_masks=False):
    if use_gt_masks and s2d.has_gt_masks:
        # Use ground truth masks instead of epipolar masks
        dynamic_mask = s2d.gt_masks.to(device)  # GT masks are foreground (dynamic) regions
        static_mask = 1.0 - dynamic_mask  # Static regions are background
        
        # Apply morphological operations for consistency
        ...
        dynamic_mask = kornia.morphology.erosion(
            dynamic_mask.float().unsqueeze(1), kernel
        ).squeeze(1)
        
        s2d.register_2d_identification(
            static_2d_mask=static_mask > 0, dynamic_2d_mask=dynamic_mask > 0
        )
        logging.info(f"Set GT masks to s2d (foreground ratio: {dynamic_mask.mean():.3f})")
    else:
        # Use epipolar masks as before
