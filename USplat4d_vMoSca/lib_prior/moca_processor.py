...
class MoCaPrep:
    ...
    def __init_depth__(
        ...
        elif self.dep_mode == "gt":
            # GT depth mode - no model loading needed
            logging.info("Using ground truth depth from mono/depth_npy folder")
            self.depth_model = None
            self.depth_process_func = None
            self.align_to_metric = False
    ...
    def compute_depth(
        ...
        if self.dep_mode == "gt":
            # Handle GT depth loading
            self._load_gt_depth(save_dir, fn_list)
            return
    ...
    def _load_gt_depth(self, save_dir, fn_list):
        """Load GT depth from mono/depth_npy folder and save in expected format"""
        import os.path as osp
        import numpy as np
        from depth_models.depth_utils import save_depth_list

        logging.info("Loading ground truth depth from mono/depth_npy folder")

        # Find the mono/depth_npy folder
        mono_depth_dir = osp.join(save_dir, "mono", "depth_npy")
        if not osp.exists(mono_depth_dir):
            raise FileNotFoundError(f"GT depth folder not found: {mono_depth_dir}")
        ...
        for fn in fn_list:
            ...
            # e.g., "000.jpg" -> "000.npy"
            base_name = osp.splitext(fn)[0]
            gt_depth_path = osp.join(mono_depth_dir, f"{base_name}.npy")

            if not osp.exists(gt_depth_path):
                raise FileNotFoundError(f"GT depth file not found: {gt_depth_path}")

            # Load GT depth (no modifications)
            gt_depth = np.load(gt_depth_path)
            dep_list.append(gt_depth)

        # Save in expected format (gt_depth directory)
        depth_save_dir = osp.join(save_dir, "gt_depth")
        save_depth_list(dep_list, fn_list, depth_save_dir)

        logging.info(f"GT depth saved to {depth_save_dir}")
        logging.info("GT depth: No boundary enhancement applied (preserving original data)")
