...
def load_objaverse_gt_pose(ws, fit_cfg):
    camera_dir = getattr(fit_cfg, "camera_json_dir", None)
    if camera_dir is None:
        camera_dir = osp.join(ws, "camera_json")
    elif not osp.isabs(camera_dir):
        camera_dir = osp.join(ws, camera_dir)

    json_paths = [
        p for p in glob.glob(osp.join(camera_dir, "*.json")) if osp.basename(p).startswith("multi")
    ]
    if not json_paths:
        raise FileNotFoundError(f"No Objaverse camera JSON found in {camera_dir}")
    ...
    json_paths.sort(key=_frame_index)
    ...
    for path in json_paths:
        ...
        T_wc = torch.tensor(cam_data["c2w_4x4"], dtype=torch.float32)
        T_wi_list.append(T_wc)
        ...
        cx = float(intr[0][2])
        cy = float(intr[1][2])
        width = float(cam_data["image_width"])
        height = float(cam_data["image_height"])
        cxcy_ratio_list.append([cx / width, cy / height])

        min_focal = min(fx, fy)
        short_side = min(width, height)
        fovdeg_list.append(math.degrees(2.0 * math.atan(short_side / (2.0 * min_focal))))

        filenames.append(osp.splitext(osp.basename(path))[0])

    gt_training_cam_T_wi = torch.stack(T_wi_list, dim=0)

    gt_training_fov = float(fovdeg_list[0])

    gt_testing_cam_T_wi_list = [gt_training_cam_T_wi.clone()]
    gt_testing_tids_list = [
        torch.arange(len(gt_training_cam_T_wi), device=gt_training_cam_T_wi.device)
    ]
    gt_testing_fns_list = [filenames]
    gt_testing_fov_list = [gt_training_fov]
    gt_testing_cxcy_ratio_list = [cxcy_ratio_list[0]]
    return (
...
def load_gt_cam(ws, fit_cfg):
    ...
    if mode == "iphone":
        ...
        return load_objaverse_gt_pose(ws, fit_cfg)
    else:
...
def static_reconstruct(ws, log_path, fit_cfg):
    ...
    if DISABLE_CAM_OPT and not INIT_GT_CAMERA_FLAG:
        raise RuntimeError(
            "disable_cam_pose_training=True requires init_gt_camera to load GT poses"
        )
    ...
    bundle_dir = osp.join(log_path, "bundle")
    ...
    if INIT_GT_CAMERA_FLAG:
        ...
        if DISABLE_CAM_OPT:
            gt_pose_ref = gt_training_cam_T_wi.detach().clone()
    ...
    if DISABLE_CAM_OPT:
        logging.info("Camera pose training disabled: freezing BA camera updates")
        ba_total_steps = 0
        ba_switch_to_ind_step = 0
        ba_depth_correction_after_step = 0
        ba_lr_cam_f = 0.0
        ba_lr_cam_q = 0.0
        ba_lr_cam_t = 0.0

    rescale_gt_cam_transl = getattr(fit_cfg, "rescale_gt_cam_transl", False)
    # if DISABLE_CAM_OPT and rescale_gt_cam_transl: # this code make rescale disabled, causing scale problem
    #     logging.warning(
    #         "rescale_gt_cam_transl overridden to False when camera pose training is disabled"
    #     )
    ...
    cams, s2d, _ = moca_solve(
        ...
        device=device,
        epi_th=EPI_TH,
        ba_total_steps=ba_total_steps,
        ba_switch_to_ind_step=ba_switch_to_ind_step,
        ba_depth_correction_after_step=ba_depth_correction_after_step,
        ba_max_frames_per_step=32,
        static_id_mode="gt_masks" if s2d.has_gt_masks else ("raft" if s2d.has_epi else "track"),
        # * robust setting
        ...
        gt_cam=cams,
        iso_focal=getattr(fit_cfg, "iso_focal", False),
        rescale_gt_cam_transl=rescale_gt_cam_transl,
        ba_lr_cam_f=ba_lr_cam_f,
        ba_lr_dep_c=getattr(fit_cfg, "ba_lr_dep_c", 0.001),
        ba_lr_dep_s=getattr(fit_cfg, "ba_lr_dep_s", 0.001),
        ba_lr_cam_q=ba_lr_cam_q,
        ba_lr_cam_t=ba_lr_cam_t,
        #
    ...
    if DISABLE_CAM_OPT and gt_pose_ref is not None:
        with torch.no_grad():
            current_pose = cams.T_wc_list().detach().cpu()
            pose_delta = (current_pose - gt_pose_ref.cpu()).abs().max()
            logging.info(
                f"Camera pose freeze check: max |ΔT_wc| = {float(pose_delta):.3e}"
            )

    if DISABLE_CAM_OPT:
        torch.save(cams.state_dict(), osp.join(bundle_dir, "bundle_cams.pth"))
        bundle_stub = {
            "dep_scale": torch.ones(cams.T, dtype=torch.float32),
            "dep_correction": torch.zeros((s2d.T, 0), dtype=torch.float32),
            "s_track": torch.zeros((s2d.T, 0, 2), dtype=torch.float32),
            "s_track_mask": torch.zeros((s2d.T, 0), dtype=torch.bool),
        }
        torch.save(bundle_stub, osp.join(bundle_dir, "bundle.pth"))
