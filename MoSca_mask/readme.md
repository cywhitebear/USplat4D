# Unofficial Update (from USplat4D Team)
Since the official version of MoSca does not support manual labeling of the FG-BG masks (at least before 06/01/2026), we provide the unofficial implementation of the FG-BG masks for MoSca.


## Installation
The usplat4d conda env supports MoSca so you may not need to reinstall the env. However, you still need to download the weights for the 2D foundation models.

Download from [here](https://drive.google.com/file/d/15tveiv7ZkvBBAN3qkkB7Zfky9d7vSqLD/view?usp=sharing) some checkpoints for the 2D foundational models if they are not HG downloadables.
...
## Usage of FG-BG mask
Add `--use_gt_masks --gt_mask_path {mask_folder_name}` when running `mosca_reconstruct.py`. For example,
```
python mosca_reconstruct.py --cfg ./profile/demo/demo_fit.yaml --ws "$ws" --use_gt_masks --gt_mask_path mask
```


# (Official Implementation) [MoSca](https://www.cis.upenn.edu/~leijh/projects/mosca/): A Modern 4D Reconstruction System for Monocular Videos
