    ...
    def photometric_fit(
        ...
        for step in tqdm(range(total_steps)):
            ...
            for _inner_loop_i, view_ind in enumerate(view_ind_list):
                ...
                # # render_dict_fg["alpha"] # [1,H,W]
                # _l_fg_mask = torch.nn.functional.l1_loss(render_dict_fg["alpha"], fg_mask_t)

                _l_fg_mask = torch.tensor(0.0,device=s_model.device)
            ...
            if (
                ...
                # Save N before densify/prune to check if it changed # MY DEBUG
                old_d_model_N = d_model.N # MY DEBUG
                apply_gs_control(
                    render_list=render_dict_list,
                    model=d_model,
                    ...
                    last_N=old_d_model_N, # MY DEBUG
                    record_flag=(not corr_exe_flag)
                    or (GS_BACKEND not in ["native_add3"]),
                )

                # Only record grad if densify/prune didn't change the point count # MY DEBUG
                # (grad tensor has old size, d_model.N has new size after densify/prune) # MY DEBUG
                if corr_exe_flag and step > dyn_node_densify_record_start_steps:
                    if d_model.N == old_d_model_N: # MY DEBUG
