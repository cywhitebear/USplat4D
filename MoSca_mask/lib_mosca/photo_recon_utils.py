...
def error_grow_dyn_model(
    ...
    # Use GT masks if available, otherwise use rendering error
    if s2d.has_gt_masks:
        logging.info("Using GT masks for dynamic mask growing")
        grow_fg_masks = s2d.gt_masks.clone().to(s2d.rgb.device)
        ...
        # viz
        imageio.mimsave(
            osp.join(viz_dir, f"gt_mask_{step}.mp4"),
        ...
        if dyn_error_grow_num_frames < T:
            ...
            grow_tids = np.arange(T)
        
        return grow_fg_masks, grow_tids
    
    # Original rendering error-based method
