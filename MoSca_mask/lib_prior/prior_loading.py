...
class Saved2D(nn.Module):
    ...
    @torch.no_grad()
    def load_gt_masks(self, gt_mask_dirname):
        """Load ground truth masks from directory"""
        gt_mask_dir = osp.join(self.ws, gt_mask_dirname)
        ...
        if len(gt_mask_fns) != len(self.frame_names):
            logging.warning(f"GT mask count {len(gt_mask_fns)} != frame count {len(self.frame_names)}")
            return self
