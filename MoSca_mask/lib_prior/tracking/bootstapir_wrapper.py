...
def prepare_fg_masks(
    ...
    vos_dir_name,
    use_gt_masks=False,
    gt_mask_path="pat_masks_gt",
):
    full_fg_mask = torch.zeros(T, tap_size, tap_size) > 0
    
    # Use GT masks if available
    if use_gt_masks:
        logging.info(f"Using GT masks from {gt_mask_path}")
        gt_mask_dir = osp.join(src, gt_mask_path)
        if osp.exists(gt_mask_dir):
            gt_mask_fns = [
            ...
            if len(gt_mask_fns) == T:
                for t, gt_mask_fn in enumerate(gt_mask_fns):
                    ...
                    resized_gt_mask = (
                        F.interpolate(
                            torch.from_numpy(gt_mask)[None, None].float(),
                            size=(tap_size, tap_size),
                            mode="nearest",
                        ).squeeze()
                        > 0
                    )
                    full_fg_mask[t] = resized_gt_mask
                logging.info(f"Loaded GT masks: {full_fg_mask.sum()} foreground pixels")
                return full_fg_mask
            else:
                logging.warning(f"GT mask count {len(gt_mask_fns)} != frame count {T}")
        else:
            logging.warning(f"GT mask directory {gt_mask_dir} not found")
    
    # Original epipolar-based method
